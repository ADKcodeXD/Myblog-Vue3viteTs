...
export const getSiteFromBGMId = (id: number, date: string) => {
    ...
    while (start < end) {
        ...
        let itemDate = new Date(bangumiData.items[mid].begin);
    ...
    while (mid>0 && mid<bangumiData.items.length) {
        ...
        let itemDate = new Date(bangumiData.items[mid].begin);
        if (itemDate.getMonth() > targetDate.getMonth() && itemDate.getFullYear()==targetDate.getFullYear()&&cnt<100) {
            mid--;
        } else if (itemDate.getMonth() < targetDate.getMonth() && itemDate.getFullYear()==targetDate.getFullYear()&&cnt<100) {
            mid++;
        } else {
            break;
        }
    ...
    while (up < bangumiData.items.length && down >= 0) {
        let upDate = new Date(bangumiData.items[up].begin);
        let downDate = new Date(bangumiData.items[down].begin);
        if (upDate.getMonth() === targetDate.getMonth()) {
            let bgmitem = bangumiData.items[up].sites.find((it: any) => {
        ...
        if (downDate.getMonth() === targetDate.getMonth()) {
            let bgmitem = bangumiData.items[down].sites.find((it: any) => {
                return it.site == "bangumi";
            })
            // 由于传进来的是number类型
            if (bgmitem&&bgmitem.id == id) {
    ...
    if (findItem) {
        ...
        if (findItem.sites.length == 0) {
            ...
            for (let i = 0; i < findItem.sites.length; i++) {
                ...
                let template = bangumiData.siteMeta[findItem.sites[i].site];
                let obj = {};
                obj['title'] = template['title'];
                let urlTemplate = template['urlTemplate'];
                let url = urlTemplate.replace('{{id}}', findItem.sites[i].id);
                obj['url'] = url;
