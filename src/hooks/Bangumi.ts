    ...
    const getWeekData = async () => {
        try {
            ...
            weekDayList.value ?. forEach(item => {
                item.items.sort((a, b) => { // 排序 首先是根据collection 的doing数据来排序 没有这个数据根据评分总数据 没有的话根据评分排序 最后根据bgm排名排序
...
export const useBroswer = (Loading : Ref < Boolean >, animeInfoList : Ref < Array < Bangumi.BroswerResult >>, page : Ref < number >) => {
 ...
 */
export const useSubjectInfo = (ids : number[]) : Promise < PromiseSettledResult < Bangumi.SubjectInfoSmall > [] >=> {
    const promises: Promise<Bangumi.SubjectInfoSmall>[] = [];
    const getPromise = (id : number): Promise < Bangumi.SubjectInfoSmall >=> {
        return new Promise(
            (resolve, reject) => {
                getSubjectInfoApi(id).then(({data}) => {
                    resolve(data.data)
                }).catch((err) => {
                    reject(err)
                });
