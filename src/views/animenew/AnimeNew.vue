  ...
  <div class="tw-p-2">
    ...
    <div v-for="(weekDay, index) in weekDayList" :key="index" class="tw-my-5">
      <h1 :class="weekDay.weekday.en">
        {{ weekDay.weekday.ja
        }}<small class="tw-font-light tw-text-sm">{{ weekDay.weekday.cn }}</small>
      </h1>
      <div class="tw-flex tw-flex-wrap tw-bg-slate-50 tw-my-2 tw-p-2">
        <div
          v-for="item in weekDay.items"
          :key="item.id"
          class="tw-w-40 tw-h-52 tw-m-3 tw-flex tw-flex-col"
        >
          <div
            class="square tw-w-40 tw-h-40 tw-relative tw-cursor-pointer tw-overflow-hidden"
          ...
          >
            ...
            <div class="pollup">
              ...
              <div class="tw-text-sm tw-break-all">开播日期：{{ item.air_date }}</div>
