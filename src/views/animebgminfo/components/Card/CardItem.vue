<template>
  <div class="box">
    <div
      class="tw-w-2/5 md:tw-w-1/3 tw-h-full tw-flex-shrink-0 tw-rounded-l-3xl tw-overflow-hidden"
    >
    ...
    <div class="tw-ml-2 tw-flex-1 tw-justify-between tw-flex tw-flex-col tw-p-2 tw-break-all">
      <div>
        ...
        <div class="tw-flex tw-justify-between">
          ...
          <div class="tw-relative">
            ...
            <div
              ref="target"
              v-if="isStatusBoxShow"
              class="tw-absolute -tw-top-20 tw-z-20 md:-tw-top-16 tw-right-0 tw-flex tw-rounded-full tw-bg-white tw-p-3 tw-drop-shadow-xl"
            >
              <ElTag
                effect="dark"
                v-for="(type, index) in tagType"
                :key="index"
                :type="type.mode"
            ...
            <ElTag
              ...
              effect="dark"
              :type="tagType[item.CollectionInfo.status.id - 1].mode"
        ...
        <div class="tw-mt-2">
          ...
          <div class="tw-flex tw-flex-wrap" v-if="item.CollectionInfo.tag">
            <ElTag
              v-for="name in item.CollectionInfo.tag.length > 3
                ? item.CollectionInfo.tag.slice(0, 3)
                : item.CollectionInfo.tag"
              :key="name"
              class="tw-m-1"
            >
              {{ name }}
      ...
      <div>
        <p v-if="item.CollectionInfo.rating !== 0">
          我的评分:<span class="tw-text-xl tw-font-bold tw-text-orange-500"
            >{{ item.CollectionInfo.rating }}/10</span
          >
        </p>
        <div class="tw-flex">
          <p>上次操作{{ formatTime(item.CollectionInfo.lasttouch * 1000, 'YYYY-MM-DD') }}</p>
