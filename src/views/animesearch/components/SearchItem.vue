<template>
  <div class="box tw-flex tw-mt-3 tw-p-3 tw-border-2 hover:tw-bg-red-200 tw-cursor-pointer" 
  @click="$router.push(`/index/animedetail/${item.id}`)">
    ...
    <div class="img tw-flex-shrink-0 tw-w-48 tw-h-48 tw-ml-3">
      <MyElimage :img="item.images.medium?item.images.medium:item.images[0]" v-if="item.images" />
      <MyElimage  v-else />
    </div>
    <div class="info tw-ml-3 tw-flex tw-flex-col tw-justify-between tw-w-full">
      <section>
        ...
        <p
          class="tw-text-sm tw-font-thin tw-text-slate-700"
          v-if="item.name_cn"
        >
          {{ item.name }}
        </p>
        <p
          class="tw-text-sm tw-text-slate-800 text-line-show-3"
          v-if="item.name_cn"
        >
          {{ item.summary }}
        </p>
...
export default defineComponent({
  props: {
    item: { type: Object as PropType<AnimeItemInfo>, default: {} },
  },
  setup(props) {
    let type = computed(() => {
      var reg = /[a-zA-Z]/g;
